...
import com.ugoodtech.umi.manager.dto.VodCreateUploadVideoResponse;
...
//    }
    ...
    public String uploadVideo(String title, String fileName, String url) throws UmiException {
        UploadURLStreamRequest request = new UploadURLStreamRequest(accessKeyId, accessKeySecret, title, fileName, url);
        request.setShowWaterMark(false);             //是否使用默认水印
     ...
     //   request.setDescription("视频描述");          //视频描述(可选)
      //  request.setCoverURL("http://cover.sample.com/sample.jpg");  //封面图片(可选)
        ...
        if (response.isSuccess()) {
            ...
            throw new UmiException(500,"视频上传失败");
        }
